        ...
        <c:when test="${totWrite >100 }">  <!-- 글 개수가 100 초과인경우 -->
	      <c:forEach   var="page" begin="1" end="10" step="1" >
	         <c:if test="${chapter >1 && page==1 }">
	          <a class="no-uline" href="${serverUrl }?action=listWrite&chapter=${chapter-1}&pageNum=${(chapter-1)*10 +1 }">&nbsp; -- </a>
	         </c:if>
	          <a class="no-uline" href="${serverUrl }?action=listWrite&chapter=${chapter}&pageNum=${page}">${(chapter-1)*10 +page } </a>
	         <c:if test="${page ==10 }">
	          <a class="no-uline" href="${serverUrl }?action=listWrite&chapter=${chapter+1}&pageNum=${chapter*10+1}">&nbsp; >></a>
	         </c:if>
	      </c:forEach>
        </c:when>
	      ...
	      <c:forEach   var="page" begin="1" end="${totWrite/10 +1}" step="1" >
	         <c:choose>
	           <c:when test="${page==pageNum }">
	             <font size=5>
	            <a class="no-uline"  href="${serverUrl }?action=listWrite&chapter=${chapter}&pageNum=${page}">${page } </a>
	            </font>
	          </c:when>
	          <c:otherwise>
	            <a class="no-uline"  href="${serverUrl }?action=listWrite&chapter=${chapter}&pageNum=${page}">${page } </a>
